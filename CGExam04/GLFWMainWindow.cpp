...
GLFWMainWindow::GLFWMainWindow(int width, int height, const char* title)
{
	...
	glfwSetFramebufferSizeCallback(pWindow, [](GLFWwindow* p, int newWidth, int newHeight)
		{
	...
	glfwSetScrollCallback(pWindow, [](GLFWwindow* p, double X, double Y)
		{
			if (AttachedCamera)
			{
	...
	glfwSetCursorPosCallback(pWindow, [](GLFWwindow* p, double xpos, double ypos)
		{
			if (AttachedCamera)
			{
				AttachedCamera->Rotate(glm::vec3(0, MOUSE_SPEED*(MouseLastPos.y - ypos), MOUSE_SPEED*(MouseLastPos.x - xpos)));
				MouseLastPos.x = (float)xpos;
				MouseLastPos.y = (float)ypos;
			}
